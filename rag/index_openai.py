# rag/index_openai.py
# Build/refresh a Chroma index using OpenAI embeddings.
...
EMBED_MODEL = os.getenv("RAG_EMBED_MODEL", "text-embedding-3-large")  # or text-embedding-3-small
...
SHOW_PROGRESS = os.getenv("RAG_PROGRESS", "1") != "0"  # kept for symmetry; not used by OpenAI
...
def _print_mode_banner() -> None:
    ...
    if not os.getenv("OPENAI_API_KEY"):
        print("[RAG] WARNING: OPENAI_API_KEY is not set; embedding calls will fail.", flush=True)
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for d in dirnames:
            ...
            if d in EXCLUDE_DIRS:
                skiplog.dirs_default.add(child_rel); continue
            if _is_ignored(child_rel, ignore_patterns):
                skiplog.dirs_ragignore.add(child_rel); continue
            kept.append(d)
        dirnames[:] = kept
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="Build/refresh local RAG index (OpenAI embeddings).")
    ...
    p.add_argument("--model", type=str, default=EMBED_MODEL, help="OpenAI embedding model (e.g., text-embedding-3-small).")
