...
"""
Normalizer (Apply) node handler.

This module consumes the Normalizer learner's bundle (port 2) and applies the stored
parameters to a new dataset, mirroring KNIME's Normalizer (Apply) node.
"""
...
FACTORY = "org.knime.base.node.preproc.normalize.NormalizerApplyNodeFactory"
HUB_URL = (
    "https://hub.knime.com/knime/extensions/org.knime.features.base/latest/"
    "org.knime.base.node.preproc.normalize.NormalizerApplyNodeFactory"
)
...
def generate_py_body(
    ...
    if len(pairs) < 2:
        raise ValueError("Normalizer (Apply) expects data and model inputs.")
    ...
    lines.append("if not isinstance(model, dict):")
    lines.append("    model = {}")
    lines.append("stats = model.get('stats')")
    lines.append("if not isinstance(stats, dict):")
    lines.append("    stats = {}")
    lines.append("mode = str(model.get('mode', 'MINMAX')).upper()")
    lines.append("columns = model.get('columns')")
    ...
    lines.append("        new_min = float(model.get('new_min', 0.0))")
    lines.append("        new_max = float(model.get('new_max', 1.0))")
    ...
    lines.append("    else:")
    lines.append("        # Unsupported modes -> no-op")
    lines.append("        pass")
