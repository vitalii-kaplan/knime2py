...
def _emit_row_agg_code(cfg: RowAggregatorSettings, node_id: str) -> List[str]:
    ...
    lines.append("        out_df = df.groupby(_cat, dropna=False).size().reset_index(name='Count')")
    lines.append("        if _grand:")
    lines.append("            _grand_df = pd.DataFrame({'Count': [len(df)]})")
    lines.append("    else:")
    lines.append("        out_df = pd.DataFrame({'Count': [len(df)]})")
