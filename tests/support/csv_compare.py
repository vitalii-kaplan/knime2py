...
def compare_csv(got_path: Path, exp_path: Path, *, rtol: float = RTOL) -> None:
    """Assert two CSVs are equal using relative tolerance for numeric cells."""
    ...
    if mismatches:
        lines = [f"First mismatches (row, col, got, exp, rel_err; math.isclose rel_tol={rtol}, abs_tol=0):"]
