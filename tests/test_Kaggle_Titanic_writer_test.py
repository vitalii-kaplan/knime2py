# Auto-generated test for KNIME workflow "Kaggle_Titanic_writer_test"
...
from support import pmml_compare
RTOL = csv_compare.RTOL

def test_roundtrip_kaggle_titanic_writer_test(output_dir: Path):
    repo_root = Path(__file__).resolve().parents[1]
    knime_proj = repo_root / "tests" / "data" / "Kaggle_Titanic_writer_test"
    out_dir = output_dir  # provided by conftest.py fixture
    expected_dir = repo_root / "tests" / "data" / "data" / "Kaggle_Titanic_writer_test"
    ...
    expected_pmmls = sorted(expected_dir.glob("*.pmml"))
    ...
    for exp in expected_csvs:
        ...
        csv_compare.compare_csv(produced, exp, rtol=RTOL, sort_columns=True)

    produced_pmml_names = {p.name for p in out_dir.glob("*.pmml")}
    for exp in expected_pmmls:
        produced = out_dir / exp.name
        assert produced.exists(), (
            f"Produced PMML not found for expected '{exp.name}'. "
            f"Produced PMMLs: {sorted(produced_pmml_names)}"
        )
        pmml_compare.compare_pmml(produced, exp)
